...
final currentTasbihSessionProvider = StreamProvider.autoDispose<TasbihSession?>(
  (ref) {
    final service = ref.watch(digitalTasbihServiceProvider);
...
final longestStreakProvider = Provider.autoDispose<int>((ref) {
  final statsAsync = ref.watch(tasbihStatsProvider);
  return statsAsync.when(
    data: (stats) => stats?.longestStreak ?? 0,
    loading: () => 0,
    error: (_, __) => 0,
  );
});
...
final recentAchievementsProvider = Provider.autoDispose<List<Achievement>>((
  ref,
) {
...
final totalAchievementPointsProvider = Provider.autoDispose<int>((ref) {
  ...
  return achievements.fold(
    0,
    (sum, achievement) => sum + achievement.pointsEarned,
  );
});

// Quick Action Providers for Common Tasbih Types
final quickTasbihProvider = Provider.autoDispose<Map<TasbihType, String>>((
  ref,
) {
  return {
...
class QuickSessionActions {
  static Future<void> startQuickTasbih(
    WidgetRef ref,
    TasbihType type, {
    int count = 33,
  }) async {
