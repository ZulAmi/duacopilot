...
void main() async {
  ...
  try {
    ...
    // Setup API keys from environment (secure configuration)
    await _setupApiKeys();
...
/// Setup API keys from secure environment configuration
Future<void> _setupApiKeys() async {
  try {
    final storage = di.sl<SecureStorageService>();

    // Setup OpenAI API key
    await _setupApiKey(storage, 'openai_api_key', EnvironmentConfig.openAiApiKey, 'OpenAI');

    // Setup Claude API key
    await _setupApiKey(storage, 'claude_api_key', EnvironmentConfig.claudeApiKey, 'Claude');

    // Setup Gemini API key
    await _setupApiKey(storage, 'gemini_api_key', EnvironmentConfig.geminiApiKey, 'Gemini');
...
/// Setup individual API key from environment
Future<void> _setupApiKey(SecureStorageService storage, String storageKey, String? envValue, String provider) async {
  try {
