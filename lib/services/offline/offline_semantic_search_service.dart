...
class OfflineSemanticSearchService {
        ...
        try {
          ...
          if (onlineResult != null) {
            ...
            return _convertToOfflineResult(
                onlineResult, queryId, SearchQuality.high);
          }
        } catch (e) {
    ...
    try {
      ...
      for (final candidate in candidates) {
        final similarity = _embeddingService.calculateSimilarity(
            queryEmbedding, candidate.vector);
  ...
  Future<void> syncWithRemote() async {
    ...
    try {
      ...
      await _prefs.setString(
          'last_offline_sync', DateTime.now().toIso8601String());
  ...
  Future<dynamic> _tryOnlineSearch({
    ...
    await _prefs.setString(
        'last_online_attempt', DateTime.now().toIso8601String());
  ...
  Future<OfflineSearchResult> _performOfflineSearch({
    ...
    final semanticMatches = await searchSimilarDuas(
        query: query, language: language, category: category);
    ...
    if (semanticMatches.isNotEmpty) {
      final result = OfflineSearchResult(
        ...
        metadata: {
          ...
          'total_candidates': await _storageService
              .getStorageStats()
              .then((s) => s['embeddings_count']),
        },
      );
    ...
    final keywordMatches =
        await _performKeywordSearch(query, language, category);
  ...
  Future<List<OfflineDuaMatch>> _performKeywordSearch(
      String query, String language, String? category) async {
    ...
    final embeddings =
        await _storageService.searchEmbeddingsByKeywords(keywords);
    ...
    for (final embedding in filteredEmbeddings.take(5)) {
      final matchedKeywords =
          _extractMatchedKeywords(query, embedding.keywords);
  ...
  List<String> _extractMatchedKeywords(
      String query, List<String> candidateKeywords) {
  ...
  Future<void> _cacheOnlineResult(
      String queryId, String query, dynamic onlineResult) async {
  ...
  OfflineSearchResult _convertToOfflineResult(
      dynamic onlineResult, String queryId, SearchQuality quality) {
  ...
  OfflineSearchResult _createEmergencyFallback(
      String queryId, String query, String language) {
    ...
    return OfflineSearchResult(
      ...
      matches: [
        OfflineDuaMatch(
          ...
          text: isArabic
              ? 'Ø¹Ø°Ø±Ø§Ù‹ØŒ Ù„Ø§ ØªÙˆØ¬Ø¯ Ù†ØªØ§Ø¦Ø¬ Ù…ØªØ§Ø­Ø© ÙÙŠ Ø§Ù„ÙˆØ¶Ø¹ Ø§Ù„Ø­Ø§Ù„ÙŠ'
